...
namespace BPIWABK.Module.BusinessObjects.Reference
{
    ...
    public class TugasFungsi : XPLiteObject
    { // Inherit from a different class to provide a custom primary key, concurrency and deletion behavior, etc. (https://documentation.devexpress.com/eXpressAppFramework/CustomDocument113146.aspx).
        public TugasFungsi(Session session)
            : base(session)
        {
        }
        ...
        {
            get => keterangan;
            set => SetPropertyValue("Keterangan", ref keterangan, value);
        }

        [Association("TugasFungsi-SOP")]
        public XPCollection<SOP> SOP
        {
            get => GetCollection<SOP>(nameof(SOP));
        }
    }
}